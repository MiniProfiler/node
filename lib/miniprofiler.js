...
var storage = new exports.storage.InMemoryStorage({
  max: 100,
  maxAge: 1000 * 60 * 60
});
...
function configure(options) {
  ...
  trivialDurationThresholdMilliseconds =
    options.trivialDurationThresholdMilliseconds ||
    trivialDurationThresholdMilliseconds;
  popupShowTimeWithChildren =
    options.popupShowTimeWithChildren || popupShowTimeWithChildren;
  popupRenderPosition = options.popupRenderPosition || popupRenderPosition;
  storage = options.storage || storage;
  resourcePath = `${
    options.resourcePath ? options.resourcePath.replace(/\/$/, '') : ''
  }${resourcePath}`;
}
...
function startProfiling(request, enabled, authorized) {
  ...
  if (enabled) {
    ...
    currentRequestExtension.stepGraph = makeStep(
      path,
      currentRequestExtension.startTime,
      null
    );
